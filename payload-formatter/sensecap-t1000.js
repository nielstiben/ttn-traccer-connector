function decodeUplink (input) {
      ...
      for (let element of message) {
        if (element.errorCode) {
          ...
          if (element.type === 'Event Status') {
            decoded.event = element['measurementValue']
          }
          if (element.type === 'Longitude') {
            decoded.longitude = element['measurementValue']
          }
          if (element.type === 'Latitude') {
  ...
  //input_str = '090000000067d456ad00390cbf030acbb055'
  // const input = {
  //   bytes: [9, 0, 0, 0, 0, 103, 212, 86, 173, 0, 57, 12, 191, 3, 10, 203, 176, 85], // Example payload
  //   fPort: 5  // Example port number
  // };
  ...
  // for (let message of output.data.messages) {
  //   for (let element of message) {
  //     console.log(element);
  //   }
  //}